...
from acfr.api.schemas import EvalRequest, QueryRequest
from acfr.eval.harness import evaluate
from acfr.rag.controller import RagController
from acfr.sources.loader import load_live_sources, load_sample_sources
from acfr.utils.config import configure_runtime
...
<script>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    input.value = '';
    btn.disabled = true;
    const thinking = addMsg('assistant', '<span style="color:#475569">&#x23F3; Thinking&hellip;</span>');
    try {
...
def ingest_live(include_fred: bool = False):
    docs = load_live_sources(
        headers={"User-Agent": os.getenv("SEC_USER_AGENT")} if os.getenv("SEC_USER_AGENT") else None,
...
def query(req: QueryRequest):
    ...
    return get_controller().answer_with_critique(req.query, top_k=req.top_k or 8)
